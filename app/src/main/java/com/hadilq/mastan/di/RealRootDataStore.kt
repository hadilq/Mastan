...
class RealRootDataStore(
    private val dataStoreLogicIo: DataStoreLogicIo,
) : RootDataStore {
    override suspend fun save(state: RootState) {
        dataStoreLogicIo.mastanDataStore.save(state)

    }

    override suspend fun load(): Result<RootState> {
        return dataStoreLogicIo.mastanDataStore.load()
    }
}